...
function formatarData(dateString) { if (!dateString) return 'N/A'; try { const [year, month, day] = dateString.split('-'); if (!year || !month || !day || year.length !== 4) return dateString; // Basic validation
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function closeDemandaDetailsModal() {
        ...
        const detailsModal = document.getElementById('demanda-details-modal');
        detailsModal?.querySelector('.bg-white')?.classList.add('scale-95','opacity-0');
        setTimeout(()=>{detailsModal?.classList.add('hidden'); currentDemandaId=null; addNoteForm?.reset();}, 300);
     }
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            if (!currentUserId || currentUserId !== user.uid) {
                ...
                if (!appInitialized && currentUserId) {
                    initializeMainApp();
                } else if (appInitialized && currentUserId) {
                    console.log("Re-authenticating user, reloading data...");
                    setupFirestoreListeners();
                    switchPage('dashboard-page');
                } else {
                    console.warn("Login detected but cannot initialize, userId:", currentUserId, "appInitialized:", appInitialized);
                }
            }
        } else {
            console.log("User is logging out or not logged in.");
            const wasLoggedIn = !!currentUserId;
            currentUser = null;
            currentUserId = null;
            console.log("currentUser and currentUserId set to null.");
            if (cidadaosListener) { cidadaosListener(); cidadaosListener = null; console.log("Unsubscribed Cidadaos listener."); }
            if (demandasListener) { demandasListener(); demandasListener = null; console.log("Unsubscribed Demandas listener."); }
            if (notesListener) { notesListener(); notesListener = null; console.log("Unsubscribed Notes listener."); }
            if (appInitialized) {
                console.log("Resetting app state (clearData, appInitialized=false).");
                allCidadaos = []; allDemandas = []; allLeaders = [];
                clearData();
                appInitialized = false;
            } else {
                console.log("App was not initialized, skipping full reset.");
            }
    ...
    function initializeMainApp() {
        if (appInitialized) { console.log("App already initialized."); return; }
        if (!currentUserId) { console.error("InitializeMainApp called without currentUserId! Aborting."); showLoginPage(); return; }
        appInitialized = true;
    ...
    function setupFirestoreListeners() {
        ...
        if (cidadaosListener) { console.log("Unsubscribing previous cidadaos listener before attaching new one."); cidadaosListener(); }
        cidadaosListener = onSnapshot(qCidadaos, (snapshot) => {
            if (!currentUserId) { console.log("Cidadaos snapshot received but user logged out. Ignoring."); return; }
        ...
        // [CORREÇÃO AQUI] Remover orderBy para evitar necessidade de índice composto
        const qDemandas = query(demandasRef, where("ownerId", "==", currentUserId));
        console.log("Setting up Firestore listener for:", 'demandas', "with ownerId:", currentUserId);
        if (demandasListener) { console.log("Unsubscribing previous demandas listener before attaching new one."); demandasListener(); }
        demandasListener = onSnapshot(qDemandas, (snapshot) => {
            if (!currentUserId) { console.log("Demandas snapshot received but user logged out. Ignoring."); return; }
            const demandasData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            // [CORREÇÃO AQUI] Ordenar no cliente
            allDemandas = demandasData.sort((a, b) => (b.createdAt?.toMillis() || 0) - (a.createdAt?.toMillis() || 0));
    ...
    function getFilteredCidadaos() {
        ...
        return allCidadaos.filter(c => {
            ...
            const leaderMatch = !leader || c.leader === leader;
    ...
    function renderCidadaos() {
        ...
        filteredCidadaos.forEach(cidadao => { // Removed sort, handled by listener potentially or keep client sort if needed
            ...
            card.innerHTML = `
                <div class="flex items-center gap-4 mb-4">
                    ${photoUrl ? `<img src="${photoUrl}" alt="${cidadao.name}" class="w-16 h-16 rounded-full object-cover bg-gray-200" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'w-16 h-16 rounded-full bg-blue-500 text-white flex items-center justify-center text-2xl font-bold\\'>${initials}</div>';">` : `<div class="w-16 h-16 rounded-full bg-blue-500 text-white flex items-center justify-center text-2xl font-bold">${initials}</div>` }
                    <div class="flex-1 min-w-0"> <h3 class="text-lg font-bold text-gray-800 truncate" title="${cidadao.name}">${cidadao.name}</h3> <p class="text-sm text-gray-600">${cidadao.type || 'N/A'}</p> </div>
                </div>
                <div class="space-y-1 text-sm text-gray-700 mb-4 flex-1">
                    <p class="flex items-center gap-2 truncate"><svg class="flex-shrink-0" ...></svg><span class="truncate" title="${cidadao.email}">${cidadao.email || '-'}</span></p>
                    <p class="flex items-center gap-2"><svg class="flex-shrink-0" ...></svg>${cidadao.phone ? formatPhone(cidadao.phone) : '-'} ${cidadao.whatsapp ? '<span class="text-green-600 font-bold">(W)</span>' : ''}</p>
                    <p class="flex items-center gap-2 truncate"><svg class="flex-shrink-0" ...></svg><span class="truncate" title="${cidadao.bairro}">${cidadao.bairro || '-'}</span></p>
                ...
                <div class="border-t pt-3 flex gap-1 justify-end flex-wrap">
                    ...
                    <button class="btn-delete text-xs bg-red-100 hover:bg-red-200 text-red-700 py-1 px-2 rounded" data-id="${cidadao.id}" data-name="${cidadao.name}"><svg ...></svg></button>
                </div>
            `;
    ...
    function renderAllDemandas() {
        ...
        const filteredDemandas = allDemandas.filter(demanda => { // allDemandas is already sorted
    ...
    function updateDashboard() {
        if(!currentUserId || !document.getElementById('dashboard-page') || document.getElementById('dashboard-page').classList.contains('hidden')) return;
    ...
    /* // CloseDetailsModal already defined above */
    ...
    /* // CloseDemandaModal already defined above */
    ...
    /* // closeDemandaDetailsModal already defined above */
