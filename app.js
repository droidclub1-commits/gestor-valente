...
try {
    ...
    // Cliente de Serviço (sbService): USADO APENAS PARA LEITURA ESTÁVEL.
    // ESTA É A CORREÇÃO: Desativa completamente a gestão de sessão (storage) para eliminar o erro "Multiple GoTrueClient".
    sbService = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE, {
        auth: {
            persistSession: false, 
            storage: {
                getItem: () => null, // Ignora a leitura de tokens
                setItem: () => {},   // Ignora a escrita de tokens
                removeItem: () => {} // Ignora a remoção de tokens
            }
        }
    });
