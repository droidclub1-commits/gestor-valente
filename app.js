...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleDeleteConfirmation() {
        ...
        try { // <-- Adicionado try aqui
            if (type === 'cidadao') {
                const dR = doc(db, 'cidadaos', id); const cData = allCidadaos.find(c=> c.id === id);
                if (cData?.photoUrl) { try { await deleteObject(ref(storage, cData.photoUrl)); } catch (e) { console.warn("Photo delete error (non-critical):", e); } } // Warn instead of error
        ...
        } catch (error) { // <-- Adicionado catch aqui
            console.error("Erro ao excluir:", error);
            showToast('Erro ao excluir.', 'error');
        } // <-- Adicionado fechamento do catch
