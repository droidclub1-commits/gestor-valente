...
try {
    ...
    if(toastContainer) { // Check if container exists before using showToast
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            if (!currentUserId || currentUserId !== user.uid) {
                ...
                if (!appInitialized && currentUserId) { initializeMainApp(); }
                else if (appInitialized && currentUserId) { console.log("Re-auth, reloading data..."); setupFirestoreListeners(); switchPage('dashboard-page'); }
                else { console.warn("Login detected but cannot init, userId:", currentUserId, "appInitialized:", appInitialized); }
            }
        } else {
            console.log("User logged out or not logged in.");
            const wasLoggedIn = !!currentUserId;
            currentUser = null; currentUserId = null; console.log("currentUser/Id set to null.");
            if (cidadaosListener) { cidadaosListener(); cidadaosListener = null; console.log("Unsub Cidadaos listener."); }
            if (demandasListener) { demandasListener(); demandasListener = null; console.log("Unsub Demandas listener."); }
            if (notesListener) { notesListener(); notesListener = null; console.log("Unsub Notes listener."); }
            if (appInitialized) { console.log("Resetting app state."); allCidadaos = []; allDemandas = []; allLeaders = []; clearData(); appInitialized = false; }
            else { console.log("App not init, skipping reset."); }
            loginPage.classList.remove('hidden'); appContainer.style.display = 'none'; console.log("Showing login page.");
        }
    });
    ...
    loginForm?.addEventListener('submit', async (e) => {
        ...
        catch (error) { console.error("Login Error:", error.code, error.message); showToast("Email ou senha inválidos.", "error"); }
        finally { setTimeout(() => { if (!auth.currentUser) { loginBtn.disabled = false; loginBtn.innerHTML = 'Entrar'; } }, 100); }
    });

    // --- Initialize Main App ---
    function initializeMainApp() {
        if (appInitialized) { console.log("Already initialized."); return; }
        if (!currentUserId) { console.error("Initialize called without userId!"); showLoginPage(); return; }
        appInitialized = true; console.log("Initializing Main App for user:", currentUserId);

        // Add Listeners
        logoBtn?.addEventListener('click', (e) => { e.preventDefault(); switchPage('dashboard-page'); });
        logoutBtn?.addEventListener('click', async () => { try { await signOut(auth); } catch (e) { console.error("Logout error:", e); showToast("Erro ao sair.", "error"); } });
        ...
        // Modal Close Buttons/Backgrounds
    ...
    function setupFirestoreListeners() {
        ...
        // [CORREÇÃO] Remover where("ownerId")
        ...
        cidadaosListener = onSnapshot(qCidadaos, (snapshot) => {
            ...
            // Filtrar líderes DEPOIS de carregar todos os cidadãos
        ...
        // [CORREÇÃO] Remover where("ownerId")
        const qDemandas = query(demandasRef, orderBy("createdAt", "desc")); // Manter orderBy aqui
        ...
        demandasListener = onSnapshot(qDemandas, (snapshot) => {
            ...
            // Ordenação já feita pelo Firestore com orderBy
            ...
            // [CORREÇÃO] Tratar erro de índice SE ele ocorrer
            if (error.code === 'failed-precondition' && error.message.includes('index')) {
                 console.warn("Firestore index required for demandas query (orderBy createdAt). Create it using the link in the previous error or remove orderBy.");
                 showToast("Índice necessário para ordenar demandas. Verifique a consola.", "warning");
                 // Fallback: Tentar carregar sem ordenar
                 if (demandasListener) demandasListener(); // Cancel this listener
                 const qDemandasFallback = query(collection(db, 'demandas')); // Query without orderBy
                 demandasListener = onSnapshot(qDemandasFallback, (snapshot) => {
                     if (!currentUserId) return;
                     const demandasData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                     allDemandas = demandasData.sort((a, b) => (b.createdAt?.toMillis() || 0) - (a.createdAt?.toMillis() || 0)); // Sort client-side as fallback
                     console.log("Firestore: Demandas updated (fallback, client sort) - ", allDemandas.length);
                     const currentPageId = document.querySelector('.page:not(.hidden)')?.id?.replace('-page', '');
                     if (currentPageId === 'demandas') renderAllDemandas();
                     if (currentPageId === 'dashboard') updateDashboard();
                 }, fallbackError => {
                     console.error("Firestore Error (Demandas Fallback):", fallbackError);
                     showToast("Erro grave ao carregar demandas.", "error");
                 });

            } else {
                console.error("Firestore Error (Demandas):", error);
    ...
    function clearData() {
        ...
        if(cidadaosGrid) cidadaosGrid.innerHTML = ''; if(allDemandasList) allDemandasList.innerHTML = '';
        ...
        updateLeaderSelects(); updateBairroFilter(); populateDemandaFilters();
        const tcEl = document.getElementById('dashboard-total-cidadaos'); const tdEl = document.getElementById('dashboard-total-demandas'); const anivEl = document.getElementById('aniversariantes-list'); const demRecEl = document.getElementById('demandas-recentes-list');
        if(tcEl) tcEl.textContent = '0'; if(tdEl) tdEl.textContent = '0'; if(anivEl) anivEl.innerHTML = ''; if(demRecEl) demRecEl.innerHTML = '';
    ...
    function getFilteredCidadaos() {
        if (!allCidadaos) return []; const searchTerm = searchInput?.value.toLowerCase() || ''; const type = filterType?.value; const bairro = filterBairro?.value; const leader = filterLeader?.value; const sexo = filterSexo?.value; const faixaEtaria = filterFaixaEtaria?.value;
        return allCidadaos.filter(c => { // No ownerId filter here
            const nameMatch = (c.name || '').toLowerCase().includes(searchTerm); const emailMatch = (c.email || '').toLowerCase().includes(searchTerm); const cpfDigits = searchTerm.replace(/\D/g,''); const cpfMatch = cpfDigits && (c.cpf || '').includes(cpfDigits); const searchMatch = !searchTerm || nameMatch || emailMatch || cpfMatch;
            const typeMatch = !type || c.type === type; const bairroMatch = !bairro || c.bairro === bairro; const leaderMatch = !leader || c.leader === leader; const sexoMatch = !sexo || (c.sexo || 'Não Informar') === sexo; const ageMatch = !faixaEtaria || getFaixaEtaria(c.dob) === faixaEtaria;
    ...
    function renderCidadaos() {
        if (!currentUserId || !cidadaosGrid) return;
        ...
        filteredCidadaos.forEach(cidadao => {
            const card = document.createElement('div'); card.className = 'bg-white p-5 rounded-lg shadow-md flex flex-col transition-shadow hover:shadow-lg'; const initials = getInitials(cidadao.name); const photoUrl = cidadao.photoUrl;
            card.innerHTML = `
                ...
                <div class="border-t pt-3 flex gap-1 justify-end flex-wrap"> <button class="btn-view-details text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 py-1 px-2 rounded" data-id="${cidadao.id}">Detalhes</button> <button class="btn-edit text-xs bg-blue-100 hover:bg-blue-200 text-blue-700 py-1 px-2 rounded" data-id="${cidadao.id}">Editar</button> <button class="btn-add-demanda text-xs bg-purple-100 hover:bg-purple-200 text-purple-700 py-1 px-2 rounded" data-id="${cidadao.id}">Demanda</button> <button class="btn-delete text-xs bg-red-100 hover:bg-red-200 text-red-700 py-1 px-2 rounded" data-id="${cidadao.id}" data-name="${cidadao.name}"><svg class="w-3.5 h-3.5 pointer-events-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button> </div>`;
            card.querySelector('.btn-view-details')?.addEventListener('click', () => openDetailsModal(cidadao.id)); card.querySelector('.btn-edit')?.addEventListener('click', () => openCidadaoModal(cidadao.id)); card.querySelector('.btn-add-demanda')?.addEventListener('click', () => openDemandaModal(cidadao.id)); card.querySelector('.btn-delete')?.addEventListener('click', () => requestDelete(cidadao.id, 'cidadao', cidadao.name));
    ...
    function renderAllDemandas() {
        if (!currentUserId || !allDemandasList) return;
        const statusFilter = demandaFilterStatus?.value; const leaderFilter = demandaFilterLeader?.value;
        const filteredDemandas = allDemandas.filter(demanda => { // allDemandas sorted client-side
            const statusMatch = !statusFilter || demanda.status === statusFilter; const solicitante = allCidadaos.find(c => c.id === demanda.cidadaoId); const leaderMatch = !leaderFilter || (solicitante && solicitante.leader === leaderFilter); return statusMatch && leaderMatch;
        });
        allDemandasList.innerHTML = ''; if (filteredDemandas.length === 0) { allDemandasList.innerHTML = '<p class="text-gray-500 text-center py-10">Nenhuma demanda encontrada.</p>'; return; }
        filteredDemandas.forEach(demanda => {
            const solicitante = allCidadaos.find(c => c.id === demanda.cidadaoId); const statusInfo = getStatusInfo(demanda.status); const item = document.createElement('div'); item.className = 'bg-white p-4 rounded-lg shadow-sm border flex justify-between items-start cursor-pointer hover:shadow-md demanda-card'; item.dataset.id = demanda.id;
            item.innerHTML = ` <div class="flex-1 min-w-0"> <h3 class="text-lg font-semibold text-gray-800 truncate" title="${demanda.title}">${demanda.title}</h3> <p class="text-sm text-gray-600"> Solicitante: <span class="font-medium text-blue-600">${solicitante ? solicitante.name : 'Desconhecido'}</span> </p> <p class="text-xs text-gray-500"> Data: ${demanda.createdAt ? demanda.createdAt.toDate().toLocaleDateString('pt-BR') : 'N/A'} ${solicitante && solicitante.leader ? ` | Líder: ${solicitante.leader}` : ''} </p> </div> <div class="flex-shrink-0 ml-4"> <span class="${statusInfo.classes}">${statusInfo.text}</span> </div>`;
            allDemandasList.appendChild(item);
        });
        allDemandasList.removeEventListener('click', handleDemandaListClick); allDemandasList.addEventListener('click', handleDemandaListClick);
    }
    ...
    // --- Dashboard ---
    function updateDashboard() { if(!currentUserId || !document.getElementById('dashboard-page') || document.getElementById('dashboard-page').classList.contains('hidden')) return; console.log("Updating dashboard UI..."); document.getElementById('dashboard-total-cidadaos').textContent = allCidadaos.length; document.getElementById('dashboard-total-demandas').textContent = allDemandas.length; updateAniversariantes(); updateDemandasRecentes(); updateCidadaosPorTipoChart(); updateDemandasPorStatusChart(); updateCidadaosPorBairroChart(); updateCidadaosPorSexoChart(); updateCidadaosPorFaixaEtariaChart(); }
    ...
    function openDetailsModal(cidadaoId) {
        ...
        detailsModal.querySelectorAll('[id^="details-"]').forEach(el => {
             const keyOriginal = el.id.replace('details-', ''); const key = keyOriginal.replace('-', ''); let value = cidadao[key] ?? '-';
    ...
    /* // closeDetailsModal defined above */
    ...
    /* // closeDemandaModal defined above */
    ...
    /* // closeDemandaDetailsModal defined above */
    ...
    /* // closeConfirmationModal defined above */
    async function handleDeleteConfirmation() {
        ...
        try {
            if (type === 'cidadao') {
                ...
                const notesRef = collection(dR, 'notes'); const notesSnap = await getDocs(notesRef); const batch = writeBatch(db); notesSnap.forEach(noteDoc => batch.delete(noteDoc.ref)); batch.delete(dR); await batch.commit();
                showToast('Demanda excluída!'); closeDemandaDetailsModal();
            }
