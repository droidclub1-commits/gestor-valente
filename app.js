...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            if (!currentUserId || currentUserId !== user.uid) {
                currentUser = user; // Assign here
                currentUserId = user.uid; // Assign here
            ...
            currentUser = null; // Assign null
            currentUserId = null; // Assign null
    ...
    loginForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                // Check current user state directly from auth after the attempt
                if (!auth.currentUser) {
                    loginBtn.disabled = false;
                    loginBtn.innerHTML = 'Entrar';
                }
                 // If successful login, onAuthStateChanged will handle hiding login page and resetting button indirectly
            }, 100);
        }
    });
    ...
    function setupFirestoreListeners() {
        ...
        const qDemandas = query(demandasRef, where("ownerId", "==", currentUserId)); // Removed orderBy
    ...
    function getFilteredCidadaos() {
        // [CORREÇÃO] Adicionar verificação se allCidadaos existe
        if (!allCidadaos) return [];
