...
try {
    // Cliente Público (sb): USADO PARA AUTH, CRUD E LEITURA SEGURA.
    // CORREÇÃO: Usar sessionStorage para evitar que o token antigo fique preso no cache.
    sb = createClient(SUPABASE_URL, SUPABASE_KEY, {
        auth: {
            persistSession: true, 
            autoRefreshToken: true,
            storage: window.sessionStorage, // <--- ÚLTIMA TENTATIVA ESTÁVEL
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInitialData() {
        ...
        try {
            // 1. Carregar Cidadãos USANDO O CLIENTE PÚBLICO (sb)
    ...
    filteredDemandas.forEach(demanda => {
        const solicitante = allCidadaos.find(c => c.id === demanda.cidadao_id);
        const statusInfo = getStatusInfo(demanda.status); // <--- CORRIGIDO: Agora usa 'demanda.status'
    ...
    recentes.forEach(demanda => { // <--- VARIÁVEL CORRETA É 'demanda'
        const solicitante = allCidadaos.find(c => c.id === demanda.cidadao_id);
        const statusInfo = getStatusInfo(demanda.status); // <--- CORRIGIDO: Usando 'demanda.status'
        const item = document.createElement('div');
        ...
        item.innerHTML = `
            ...
            <p class="text-sm text-gray-600">
                ...
                ${demanda.created_at ? new Date(demanda.created_at).toLocaleDateString('pt-BR') : 'N/A'}
            </p>
        `;
        item.addEventListener('click', () => { openDemandaDetailsModal(demanda.id); });
        listEl.appendChild(item);
