...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA DE LOGIN ---
    let appInitialized = false; // Moved flag here to be accessible by onAuthStateChanged
        ...
        if (user) {
            ...
            if (!currentUserId || currentUserId !== user.uid) { // Check if user actually changed or is logging in
                ...
                if (!appInitialized && currentUserId) { // Ensure userId is set before initializing
                    ...
                    // If app already initialized (e.g., user re-authenticated), just reload data
                    console.log("Re-authenticating user, reloading data...");
                    setupFirestoreListeners();
                    switchPage('dashboard-page'); // Go to dashboard on re-auth
                } else {
            ...
            const wasLoggedIn = !!currentUserId; // Check if we *were* logged in
            ...
            // Clear listeners only if they exist
            ...
            // Reset app state only if it was previously initialized
            if (appInitialized) {
