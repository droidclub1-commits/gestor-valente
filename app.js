...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleAddNoteSubmit(e) { // <-- This function was incomplete
        e.preventDefault();
        ...
        try {
            const demandaRef = doc(db, 'demandas', currentDemandaId); // Path to demanda doc
            const notesRef = collection(demandaRef, 'notes'); // Path to subcollection
            await addDoc(notesRef, { text, createdAt: Timestamp.now(), author: auth.currentUser.email || "User" });
            await updateDoc(demandaRef, { updatedAt: Timestamp.now() }); // Update parent timestamp
    ...
    async function handleDeleteConfirmation() {
        ...
        try {
            if (type === 'cidadao') {
                ...
                if (cData?.photoUrl) { try { await deleteObject(ref(storage, cData.photoUrl)); } catch (e) { console.error("Photo delete error:", e); } }
                await deleteDoc(dR); showToast('Cidadão excluído!');
            } else if (type === 'demanda') {
                const dR = doc(db, 'demandas', id);
                // Delete notes subcollection (best effort client-side)
        ...
        } catch (error) { console.error("Erro:", error); showToast('Erro ao excluir.', 'error'); }
