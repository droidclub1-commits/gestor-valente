...
// SUPABASE_SERVICE_ROLE e sbService foram removidos por SEGURANÇA e ESTABILIDADE.

const { createClient } = supabase;
let sb, user = null; // Removido sbService

try {
    // Cliente Público (sb): ÚNICO CLIENTE USADO PARA TUDO.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------------------------------------
    // MONITORAMENTO DE SESSÃO ÚNICO E ESTÁVEL
    // ----------------------------------------------------------------------------------
    sb.auth.onAuthStateChange(async (event, session) => {
        if (session && session.user) {
            ...
            // Se a sessão inicial for nula, garante que a tela de login apareça
            user = null;
            appInitialized = false;
            loginPage.classList.remove('hidden');
            appContainer.style.display = 'none';
     ...
     * AGORA USA O CLIENTE PÚBLICO SEGURO (sb)
     */
    async function loadInitialData() {
    if (!user) return;
    
    try {
        const session = await sb.auth.getSession(); // Obtém a sessão atual
        const accessToken = session.data.session?.access_token;
        
        if (!accessToken) throw new Error("Token de acesso não encontrado. Sessão inválida.");

        // Chama o servidor Netlify para buscar os dados de forma segura
        const response = await fetch('/.netlify/functions/load-data', {
            method: 'GET',
            headers: {
                // Envia o token para que o servidor possa verificar a identidade do usuário
        ...
        if (!response.ok) {
            ...
            // Lança o erro para o bloco 'catch' do initializeMainApp
            throw new Error(err.error || `Erro ${response.status}: Falha ao buscar dados do servidor.`);
        }
        
        const data = await response.json();
        
        // Atribui os dados FILTRADOS e SEGUROS vindos do servidor
        allCidadaos = data.cidadaos;
        allDemandas = data.demandas;
        
        // 3. Atualizar UI (Resto do código de UI permanece o mesmo)
