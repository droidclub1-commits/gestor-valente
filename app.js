...
const EDGE_FUNCTION_URL = 'https://memsmrsntvkneyylggto.supabase.co/functions/v1/manage-users';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function manageSessionOnLoad() {
        ...
        if (session) {
            ...
            loginPage.classList.add('hidden');
            appContainer.style.display = 'flex';
            // Não chama initializeMainApp aqui — o onAuthStateChange
            // dispara com INITIAL_SESSION e faz isso de forma segura
        } else {
            user = null;
            loginPage.classList.remove('hidden');
            appContainer.style.display = 'none';
    ...
    async function initializeMainApp() {
        ...
        logoutBtn.addEventListener('click', async () => {
            try {
                ...
                // Reseta estado global antes do signOut
                allCidadaos = []; allDemandas = []; allLeaders = [];
                totalCidadaosCount = 0; cidadaosServerOffset = 0;
                userRole = null;
                appInitialized = false;
                await sb.auth.signOut();
                // onAuthStateChange trata o resto (esconde app, mostra login)
            } catch (error) {
