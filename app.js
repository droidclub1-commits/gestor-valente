...
// (Vá para Configurações > API no seu projeto Supabase para encontrar isso)
...
let viewingDemandaId = null; // Renomeado de 'viewingDemandaId' para consistência
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    sb.auth.onAuthStateChange(async (_event, session) => {
        if (session && session.user) {
            ...
            await initializeMainApp(); // Inicializar a aplicação principal
        } else {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // O onAuthStateChange vai tratar da mudança de tela
        } catch (error) {
    ...
    // ... (Todas as suas variáveis de UI (logoBtn, logoutBtn, etc.) permanecem as mesmas) ...
    // (Declarei elas aqui para o código não quebrar)
    ...
    async function initializeMainApp() {
        ...
        // Captura de Elementos (Idêntico ao seu código anterior)
        ...
        // ** AGORA TODAS AS FUNÇÕES ESTÃO DEFINIDAS CORRETAMENTE **
        ...
        await loadInitialData(); // Função que substitui os listeners
    ...
    /**
     * NOVO: Carrega todos os dados iniciais do Supabase
     */
    async function loadInitialData() {
        ...
        try {
            ...
            const { data: cidadaosData, error: cidadaosError } = await sb
                .from('cidadaos')
                .select('*')
                .order('name', { ascending: true }); // Já vem ordenado!
            ...
            showToast("Erro fatal ao carregar dados.", "error");
        }
    }
    ...
    async function handleCidadaoFormSubmit(e) {
        ...
        try {
            ...
            const cidadaoData = {
                ...
                // --- CORREÇÃO ESTÁ AQUI ---
                localtrabalho: cidadaoLocalTrabalho.value, // (era localTrabalho)
                photourl: photoUrl || null,                 // (era photoUrl)
                // --- FIM DA CORREÇÃO ---
    ...
    // -------------------------------------------------------------------------
    //  RESTO DO SEU CÓDIGO (NÃO PRECISA MUDAR NADA!)
    //  Todas as funções de UI, filtros, gráficos, mapas e relatórios
    //  continuam funcionando, pois elas leem das variáveis globais
    //  `allCidadaos` e `allDemandas`, que nós continuamos a preencher.
    // -------------------------------------------------------------------------
    
    // As funções abaixo são IDÊNTICAS ao seu arquivo original.
    // Elas apenas leem de arrays, então não precisam de alteração.
    ...
    // --- LÓGICA DO DASHBOARD (IDÊNTICO, SÓ MUDAR FORMATO DE DATA) ---
    ...
    // --- LÓGICA DO FORMULÁRIO CIDADÃO (IDÊNTICO) ---
    async function openCidadaoModal(cidadaoId = null) {
        ...
        if (cidadaoId) {
            ...
            if (cidadao) {
                ...
                // --- CORREÇÃO ESTÁ AQUI ---
                cidadaoLocalTrabalho.value = cidadao.localtrabalho || ''; // (era localTrabalho)
                cidadaoPhotoUrl.value = cidadao.photourl || '';         // (era photoUrl)
                // --- FIM DA CORREÇÃO ---
    ...
    // --- LÓGICA DE DETALHES DO CIDADÃO (IDÊNTICO) ---
    function openDetailsModal(cidadaoId) {
        ...
        // --- CORREÇÃO ESTÁ AQUI ---
        if (cidadao.photourl) { // (era photoUrl)
        ...
        // --- CORREÇÃO ESTÁ AQUI ---
        document.getElementById('details-local-trabalho').textContent = cidadao.localtrabalho || 'Não informado'; // (era localTrabalho)
        // --- FIM DA CORREÇÃO ---
    ...
    function openDemandaModal(cidadaoId = null) {
        ...
        // 2b. Resetar display das opções (caso tenham sido filtradas antes)
        allCidadaos.sort((a,b) => a.name.localeCompare(b.name)).forEach(c => {
            const option = document.createElement('option');
            option.value = c.id;
            option.textContent = c.name;
            // option.style.display = ''; // Garantir que está visível (opcional, mas bom)
            demandaCidadaoSelect.appendChild(option);
        });

        // 2c. Popular a lista e resetar o display (forma mais segura)
        // A lógica acima é melhor que o innerHTML
        ...
        // (Fazemos isso aqui para garantir que os elementos existem)
        ...
        // Usamos 'onkeyup' em vez de addEventListener para evitar múltiplos listeners
    ...
    // *** INÍCIO DA CORREÇÃO ***
    // As funções foram movidas para fora de 'openDemandaModal' e adicionadas
    ...
    async function openMapModal(cidadaosToPlot = null) {
        ...
        if (!Array.isArray(cidadaos)) {
            console.error("openMapModal foi chamado com um valor não-iterável:", cidadaos);
            return;
        }
