...
const SUPABASE_SERVICE_ROLE = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndwZWVmbnJuY2txeG9sYmllaGlxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MzQzNzI3OSwiZXhwIjoyMDc5MDEzMjc5fQ.NBUkeop8Bujm70_XaxRZV4roDE8d2uSRZY28oiVmPQk'; // SUA SERVICE ROLE INJETADA

const { createClient } = supabase;
let sb, sbService, user = null;

try {
    // Cliente Público (sb): Usado para Auth, INSERT, UPDATE, DELETE (sujeito a RLS)
    sb = createClient(SUPABASE_URL, SUPABASE_KEY, {
        auth: {
            persistSession: true, // Manter persistência estável
            autoRefreshToken: true
        }
    });

    // Cliente de Serviço (sbService): Usado apenas para SELECT (Leitura Global Estável)
    sbService = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE, {
        auth: {
            persistSession: false, // Não precisamos de persistência para esta instância
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------------------------------------
    // NOVO: Monitorar estado da autenticação (GARANTE LOGOUT E LOGIN CORRETO)
    // ----------------------------------------------------------------------------------
