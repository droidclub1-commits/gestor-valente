...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- [MOVIDO PARA CIMA] Funções de Fechar Modais ---
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            // User is logged in
            if (!currentUserId || currentUserId !== user.uid) { // Check if user actually changed or is logging in
                currentUser = user;
                ...
                // Initialize app or reload data
                if (!appInitialized && currentUserId) { // Ensure userId is set before initializing
                    initializeMainApp();
    ...
    loginForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Restore button only if login failed (success is handled by onAuthStateChanged)
             // Check auth state directly after attempt, before relying on async onAuthStateChanged
            setTimeout(() => { // Small delay to allow potential onAuthStateChanged to trigger first
                if (!auth.currentUser) {
    ...
    // Moved appInitialized declaration outside initializeMainApp

    function initializeMainApp() {
        if (appInitialized) {
             console.log("initializeMainApp called but already initialized.");
             return;
        }
        if (!currentUserId) {
            console.error("InitializeMainApp called without currentUserId! Aborting initialization.");
            showLoginPage(); // Ensure login page is shown if something went wrong
            return;
        }

        appInitialized = true; // Set flag
    ...
    function setupFirestoreListeners() {
        ...
        cidadaosListener = onSnapshot(qCidadaos, (snapshot) => {
            if (!currentUserId) { console.log("Cidadaos snapshot received but user logged out. Ignoring."); return; } // Check if user logged out
        ...
        demandasListener = onSnapshot(qDemandas, (snapshot) => {
            if (!currentUserId) { console.log("Demandas snapshot received but user logged out. Ignoring."); return; } // Check if user logged out
        ...
        filteredCidadaos.forEach(cidadao => {
            ...
            card.innerHTML = `
                <div class="flex items-center gap-4 mb-4">
                    ${photoUrl ? `<img src="${photoUrl}" ...>` : `<div class="w-16 ...">${initials}</div>`}
                    <div class="flex-1 min-w-0"> ... </div>
                </div>
                <div class="space-y-1 ... flex-1"> ... </div>
                <div class="border-t pt-3 flex gap-1 justify-end flex-wrap"> ... buttons ... </div>`; // Simplified for brevity
    ...
    function renderAllDemandas() {
        ...
        filteredDemandas.forEach(demanda => {
            ...
            item.innerHTML = `
                <div class="flex-1 min-w-0"> ... </div>
                <div class="flex-shrink-0 ml-4"> ... </div>`; // Simplified
