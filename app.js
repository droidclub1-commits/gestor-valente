...
try {
    // OPÇÃO A: FORÇAR O USO DE COOKIES (Mais estável para recarregamento)
    sb = createClient(SUPABASE_URL, SUPABASE_KEY, {
      auth: {
        storageKey: 'supabase-auth-token',
        persistSession: true,
        storage: window.sessionStorage, // Usar sessionStorage é mais seguro
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----------------------------------------------------------------------------------
    // NOVO: Monitorar estado da autenticação (APENAS PARA LOGIN/LOGOUT E SINCRONIZAÇÃO)
    // ----------------------------------------------------------------------------------
    sb.auth.onAuthStateChange(async (event, session) => {
        if (event === 'SIGNED_IN' || event === 'INITIAL_SESSION') {
            // Se logar com sucesso, a função manageSessionOnLoad() já iniciou tudo, 
            // mas garante que a tela está correta.
            user = session.user;
            loginPage.classList.add('hidden');
            appContainer.style.display = 'flex';
            
            // Tenta iniciar se ainda não estiver inicializado (caso de login via form)
            if (!appInitialized) {
