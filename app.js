...
const firebaseConfig = {
  ...
  storageBucket: "gestor-valente-crm.appspot.com", // Verifique se este é o correto
...
try {
    ...
    // Tenta exibir um alerta se a função showToast não estiver disponível ainda
...
let notesListener = null; // Listener específico para notas da demanda aberta
let itemToDelete = { id: null, type: null }; // Para modal de confirmação
...
document.addEventListener('DOMContentLoaded', () => {
    // DOM Elements Cache (cache elements inside DOMContentLoaded)
    ...
    // ... cache other DOM elements similarly ...
    ...
    const modalContent = document.getElementById('modal-content'); // Might need more specific ID if multiple modals use it
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            currentUser = user; currentUserId = user.uid; // Store user globally
            loginPage.classList.add('hidden');
            appContainer.style.display = 'flex';
            if (!appInitialized) initializeMainApp(); // Initialize only once
        } else {
    ...
    function initializeMainApp() {
        ...
        logoutBtn?.addEventListener('click', async () => { try { await signOut(auth); appInitialized = false; } catch (e) { console.error("Logout error:", e); showToast("Erro ao sair.", "error"); } });
        ...
        // Filters and Search
        searchInput?.addEventListener('input', () => renderCidadaos()); // Pass no args, uses global allCidadaos
        ...
        demandaFilterStatus?.addEventListener('change', () => renderAllDemandas()); // Pass no args
        ...
        // Confirmation Modal Buttons (Listeners added in requestDelete)
        cancelDeleteBtn?.addEventListener('click', closeConfirmationModal);
        confirmDeleteBtn?.addEventListener('click', handleDeleteConfirmation); // Re-added direct listener as fallback
    ...
    function setupFirestoreListeners() {
        ...
        const cidadaosRef = collection(db, 'cidadaos'); // Root collection
        const qCidadaos = query(cidadaosRef); // Potentially add where("ownerId", "==", currentUserId) if needed by rules
        console.log("Setting up Firestore listener for:", 'cidadaos');
        if (cidadaosListener) cidadaosListener(); // Unsubscribe previous
        cidadaosListener = onSnapshot(qCidadaos, (snapshot) => {
            ...
            // Re-render current page if relevant
            ...
            if (currentPageId === 'demandas') populateDemandaFilters(); // Update leader filter only
        ...
        const demandasRef = collection(db, 'demandas'); // Root collection
        const qDemandas = query(demandasRef, orderBy("createdAt", "desc")); // Potentially add where("ownerId", "==", currentUserId)
        console.log("Setting up Firestore listener for:", 'demandas');
        if (demandasListener) demandasListener(); // Unsubscribe previous
        demandasListener = onSnapshot(qDemandas, (snapshot) => {
            ...
            if (currentPageId === 'dashboard') updateDashboard(); // Update lists/charts on dashboard
    ...
    function renderCidadaos() { /* ... same as your pasted version, uses getFilteredCidadaos ... */
        const cidadaosGrid = document.getElementById('cidadaos-grid');
        ...
        filteredCidadaos.forEach(cidadao => {
            ...
            `;
            // Add events within the loop (event delegation is better but this matches original code)
    ...
    function renderAllDemandas() { /* ... same as your pasted version, uses getFilteredDemandas ... */
        const allDemandasList = document.getElementById('all-demandas-list');
        if (!allDemandasList) return;
        const statusFilter = document.getElementById('demanda-filter-status').value;
        const leaderFilter = document.getElementById('demanda-filter-leader').value; // Filter by name
        ...
        filteredDemandas.forEach(demanda => {
            ...
            item.className = 'bg-white p-4 rounded-lg shadow-sm border flex justify-between items-start cursor-pointer hover:shadow-md demanda-card'; // Added demanda-card class
            item.dataset.id = demanda.id; // Store ID on the card itself
            item.innerHTML = `
                <div class="flex-1 min-w-0"> {/* Added min-w-0 */}
                    ...
                    <p class="text-xs text-gray-500">
                        ...
                        ${solicitante && solicitante.leader ? ` | Líder: ${solicitante.leader}` : ''}
                    </p>
                </div>
                <div class="flex-shrink-0 ml-4"> {/* Added margin */}
                    <span class="${statusInfo.classes}">${statusInfo.text}</span>
                </div>
            `;
            allDemandasList.appendChild(item);
        });
        // Add single listener to the list container (Event Delegation)
        allDemandasList.removeEventListener('click', handleDemandaListClick); // Remove old listener
        allDemandasList.addEventListener('click', handleDemandaListClick);
    }
    // Handle clicks on the demands list
    function handleDemandaListClick(e) {
    ...
    // --- Update Leader/Bairro Selects ---
    function updateLeaderSelects() { /* ... same as your pasted version ... */
        const selects = [ document.getElementById('cidadao-leader'), document.getElementById('filter-leader'), document.getElementById('demanda-filter-leader') ];
        selects.forEach(select => { if (!select) return; const currentValue = select.value; select.innerHTML = `<option value="">${select.id === 'cidadao-leader' ? 'Nenhuma' : 'Filtrar Liderança'}</option>`; allLeaders.forEach(leader => { const option = document.createElement('option'); option.value = leader.name; /* Store/Filter by name */ option.textContent = leader.name; select.appendChild(option); }); select.value = currentValue; });
    }
    function updateBairroFilter() { /* ... same as your pasted version ... */
        const filterBairro = document.getElementById('filter-bairro'); if (!filterBairro) return; const currentValue = filterBairro.value; const bairros = [...new Set(allCidadaos.map(c => c.bairro).filter(Boolean))].sort(); filterBairro.innerHTML = '<option value="">Filtrar Bairro</option>'; bairros.forEach(bairro => { const option = document.createElement('option'); option.value = bairro; option.textContent = bairro; filterBairro.appendChild(option); }); filterBairro.value = currentValue;
    }
    ...
    // --- Dashboard ---
    function updateDashboard() { /* ... same as your pasted version ... */
    ...
    // --- Chart Functions --- (Ensure canvas IDs match HTML)
    function updateCidadaosPorTipoChart() { const ctx = document.getElementById('cidadaos-por-tipo-chart')?.getContext('2d'); if(!ctx) return; const data = allCidadaos.reduce((a,c)=>{const t=c.type||'N/D';a[t]=(a[t]||0)+1;return a},{}); const labels=Object.keys(data); const values=Object.values(data); if(cidadaosPorTipoChartInstance)cidadaosPorTipoChartInstance.destroy(); cidadaosPorTipoChartInstance=new Chart(ctx,{type:'pie',data:{labels,datasets:[{data:values,backgroundColor:['#3B82F6','#8B5CF6','#10B981','#F59E0B','#6B7280'].slice(0,labels.length)}] },options:{responsive:true,maintainAspectRatio:false}}); }
    function updateDemandasPorStatusChart() { const ctx = document.getElementById('demandas-por-status-chart')?.getContext('2d'); if(!ctx) return; const data = allDemandas.reduce((a,d)=>{a[d.status]=(a[d.status]||0)+1;return a;},{pending:0,inprogress:0,completed:0}); const labels=Object.keys(data).map(s=>getStatusInfo(s).text); const values=Object.values(data); const colors=Object.keys(data).map(s=>getStatusInfo(s).color); if(demandasPorStatusChartInstance)demandasPorStatusChartInstance.destroy(); demandasPorStatusChartInstance=new Chart(ctx,{type:'doughnut',data:{labels,datasets:[{data:values,backgroundColor:colors}]},options:{responsive:true,maintainAspectRatio:false}}); }
    function updateCidadaosPorBairroChart() { const ctx = document.getElementById('cidadaos-por-bairro-chart')?.getContext('2d'); if(!ctx) return; const data = allCidadaos.reduce((a,c)=>{const b=c.bairro||'N/I';a[b]=(a[b]||0)+1;return a;},{}); const sorted = Object.entries(data).sort(([,a],[,b])=>b-a).slice(0,10); const labels=sorted.map(([b])=>b); const values=sorted.map(([,c])=>c); if(cidadaosPorBairroChartInstance)cidadaosPorBairroChartInstance.destroy(); cidadaosPorBairroChartInstance=new Chart(ctx,{type:'bar',data:{labels,datasets:[{label:'Cidadãos',data:values,backgroundColor:'#10B981'}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false}}}}); }
    function updateCidadaosPorSexoChart() { const ctx = document.getElementById('cidadaos-por-sexo-chart')?.getContext('2d'); if(!ctx) return; const data = allCidadaos.reduce((a,c)=>{const s=c.sexo||'N/I';a[s]=(a[s]||0)+1;return a;},{'Masculino':0,'Feminino':0,'Outro':0,'Não Informar':0}); const labels=Object.keys(data).filter(k=>data[k]>0); const values=labels.map(l=>data[l]); if(cidadaosSexoChartInstance)cidadaosSexoChartInstance.destroy(); cidadaosSexoChartInstance=new Chart(ctx,{type:'pie',data:{labels,datasets:[{data:values,backgroundColor:['#3B82F6','#EC4899','#F59E0B','#6B7280'].slice(0,labels.length)}]},options:{responsive:true,maintainAspectRatio:false}}); }
    function updateCidadaosPorFaixaEtariaChart() { const ctx = document.getElementById('cidadaos-por-faixa-etaria-chart')?.getContext('2d'); if(!ctx) return; const faixas={'0-17':0,'18-25':0,'26-35':0,'36-50':0,'51-65':0,'66+':0,'N/A':0}; allCidadaos.forEach(c=>{faixas[getFaixaEtaria(c.dob)]++;}); const labels=Object.keys(faixas); const values=Object.values(faixas); if(cidadaosFaixaEtariaChartInstance)cidadaosFaixaEtariaChartInstance.destroy(); cidadaosFaixaEtariaChartInstance=new Chart(ctx,{type:'bar',data:{labels,datasets:[{label:'Cidadãos',data:values,backgroundColor:'#8B5CF6'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}}); }

    // --- Cidadao Form Logic ---
    async function handleCidadaoFormSubmit(e) { /* ... same as your pasted version ... */
        ...
        try { if (file) { const fP = `photos/${currentUserId}/${Date.now()}_${file.name}`; const sR = ref(storage, fP); const sn = await uploadBytes(sR, file); pU = await getDownloadURL(sn.ref); }
            const children = getChildrenData(); // Use helper
    ...
    async function handleCEPBlur(e) { /* ... same as your pasted version ... */
    ...
    function updateChildrenInputs(type, childrenData = null) { /* ... same as your pasted version ... */
        const countInput = type === 'filho' ? cidadaoSons : cidadaoDaughters;
        const count = parseInt(countInput.value) || 0;
        const containerId = type === 'filho' ? 'sons-inputs' : 'daughters-inputs';
        const label = type === 'filho' ? 'Filho' : 'Filha';
        let container = document.getElementById(containerId);
        if (!container && count > 0) { // Only create container if needed
            container = document.createElement('div');
            container.id = containerId;
            container.className = 'mt-4 space-y-3 p-4 bg-gray-50 rounded-lg border';
            childrenDetailsContainer.appendChild(container);
        } else if (container && count === 0) {
             container.remove(); // Remove container if count is zero
             return;
        } else if (!container && count === 0) {
            return; // No container needed
        }

        container.innerHTML = ''; // Clear existing inputs within the specific container

        if (count > 0) {
            container.innerHTML += `<h4 class="font-medium text-gray-700 mb-2">${label}s (${count}):</h4>`;
             // Find relevant children from the potentially combined list
             const relevantChildren = (childrenData || []).filter(c => c.type === type).sort((a,b)=> a.index - b.index);

            for (let i = 0; i < count; i++) {
                 const existingChild = relevantChildren[i]; // Get child data based on sorted index
                container.innerHTML += `
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 border-t pt-3 first:border-t-0 first:pt-0">
                        <div>
                            <label class="block text-xs font-medium text-gray-600">${label} ${i + 1} - Nome</label>
                            <input type="text" data-type="${type}" data-index="${i}" data-field="name" class="w-full border border-gray-300 p-2 rounded-lg mt-1 text-sm child-input" value="${existingChild?.name || ''}">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600">${label} ${i + 1} - Data Nasc.</label>
                            <input type="date" data-type="${type}" data-index="${i}" data-field="dob" class="w-full border border-gray-300 p-2 rounded-lg mt-1 text-sm child-input" value="${existingChild?.dob || ''}">
                        </div>
                    </div>
                `;
            }
    ...
    function getChildrenData() { /* ... same as your pasted version ... */
        const children = []; const inputs = childrenDetailsContainer.querySelectorAll('input[data-type].child-input'); inputs.forEach(input => { const type = input.dataset.type; const index = parseInt(input.dataset.index, 10); const field = input.dataset.field; const value = input.value.trim(); let child = children.find(c => c.type === type && c.index === index); if (!child) { child = { type, index }; children.push(child); } child[field] = value; }); return children.filter(c => c.name); // Only return children with names
     }

    // --- Details Modal ---
    function openDetailsModal(cidadaoId) { /* ... same as your pasted version, uses global allCidadaos ... */
        currentCidadaoId = cidadaoId; // Reuse global variable? Or use a dedicated one? Let's use currentCidadaoId for now.
        ...
        const photoEl = document.getElementById('details-photo'); const initials = getInitials(cidadao.name); if (cidadao.photoUrl) { photoEl.innerHTML = `<img src="${cidadao.photoUrl}" alt="${cidadao.name}" class="w-24 h-24 rounded-full object-cover bg-gray-200" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'w-24 h-24 rounded-full bg-blue-500 text-white flex items-center justify-center text-4xl font-bold\\'>${initials}</div>';">`; } else { photoEl.innerHTML = `<div class="w-24 h-24 rounded-full bg-blue-500 text-white flex items-center justify-center text-4xl font-bold">${initials}</div>`; }
        document.getElementById('details-name').textContent = cidadao.name; document.getElementById('details-type').textContent = cidadao.type; document.getElementById('details-email').textContent = cidadao.email || '-'; document.getElementById('details-phone').textContent = cidadao.phone ? `${formatPhone(cidadao.phone)} ${cidadao.whatsapp ? '(W)' : ''}` : '-'; const addr = [cidadao.logradouro, cidadao.numero, cidadao.complemento, cidadao.bairro, cidadao.cidade, cidadao.estado, cidadao.cep?`CEP: ${formatCEP(cidadao.cep)}`:null].filter(Boolean).join(', ')||'-'; document.getElementById('details-address').textContent = addr; document.getElementById('details-cpf').textContent = cidadao.cpf?formatCPF(cidadao.cpf):'-'; document.getElementById('details-rg').textContent = cidadao.rg || '-'; document.getElementById('details-voterid').textContent = cidadao.voterId || '-'; document.getElementById('details-dob').textContent = cidadao.dob ? formatarData(cidadao.dob) : '-'; document.getElementById('details-sexo').textContent = cidadao.sexo || '-'; document.getElementById('details-profissao').textContent = cidadao.profissao || '-'; document.getElementById('details-local-trabalho').textContent = cidadao.localTrabalho || '-'; const leaderName = allLeaders.find(l=>l.name === cidadao.leader)?.name; document.getElementById('details-leader').textContent = leaderName || cidadao.leader || '-';
    ...
    function closeDetailsModal() { /* ... same as your pasted version ... */
    ...
    // --- Demanda Form/Modals ---
    function openDemandaModal(cidadaoId = null) { /* ... same as your pasted version ... */
    ...
    function closeDemandaModal() { /* ... same as your pasted version ... */ demandaModal?.classList.add('hidden'); }
    async function handleDemandaFormSubmit(e) { /* ... same as your pasted version, uses root collection ... */
    ...
    async function openDemandaDetailsModal(demandaId) { /* ... same as your pasted version ... */
    ...
    function closeDemandaDetailsModal() { /* ... same as your pasted version ... */
    ...
    async function updateDemandaStatus(demandaId, newStatus) { /* ... same as your pasted version, uses root collection + notes subcollection */
    ...
    function loadDemandaNotes(demandaId) { /* ... same as your pasted version, uses notes subcollection */
    ...
    async function handleAddNoteSubmit(e) { /* ... COIGO COMPLETO FALTANTE AQUI ... */
        e.preventDefault();
        const uid = auth.currentUser?.uid; // Use optional chaining
        if (!uid || !currentDemandaId) {
             console.warn("User not logged in or no demanda selected for adding note.");
             return;
        }
        ...
        if (!text) {
             showToast("Escreva o acompanhamento antes de adicionar.", "warning");
             return;
        }
        ...
        try {
            const demandaRef = doc(db, 'demandas', currentDemandaId); // Reference to the parent demanda document
            const notesRef = collection(demandaRef, 'notes'); // Reference to the 'notes' subcollection

            await addDoc(notesRef, {
                text: text,
                createdAt: Timestamp.now(), // Use server timestamp
                author: auth.currentUser.email || "Utilizador" // Get email if available
            });

            // Optionally update the 'updatedAt' timestamp on the parent demanda
            await updateDoc(demandaRef, {
                 updatedAt: Timestamp.now()
            });


            newNoteTextElement.value = ''; // Clear the textarea
            showToast("Acompanhamento adicionado!", "success");
            // The onSnapshot listener in loadDemandaNotes will update the UI
     ...
     } // <-- Fechamento da função handleAddNoteSubmit


    // --- Deletion Logic ---
    function requestDelete(itemId, type, name = '') { /* ... same as your pasted version ... */
        itemToDelete = { id: itemId, type: type }; const modal = document.getElementById('confirmation-modal'); const title = document.getElementById('confirmation-title'); const message = document.getElementById('confirmation-message'); const itemIdentifier = name || (type === 'demanda' ? allDemandas.find(d=>d.id===itemId)?.title : '') || 'este item'; if (type === 'cidadao') { title.textContent = 'Excluir Cidadão'; message.textContent = `Excluir "${itemIdentifier}"? Demandas associadas NÃO serão excluídas.`; } else if (type === 'demanda') { title.textContent = 'Excluir Demanda'; message.textContent = `Excluir "${itemIdentifier}"?`; } modal?.classList.remove('hidden');
     }
    function closeConfirmationModal() { /* ... same as your pasted version ... */ document.getElementById('confirmation-modal')?.classList.add('hidden'); itemToDelete = { id: null, type: null }; }
    async function handleDeleteConfirmation() { /* ... same as your pasted version, uses root collections ... */
        ...
        try { if (type === 'cidadao') { const dR = doc(db, 'cidadaos', id); const cData = allCidadaos.find(c=> c.id === id); if (cData?.photoUrl) { try { await deleteObject(ref(storage, cData.photoUrl)); } catch (e) { console.error("Photo delete error:", e); } } await deleteDoc(dR); showToast('Cidadão excluído!'); } else if (type === 'demanda') { const dR = doc(db, 'demandas', id); // Logic to delete notes subcollection would go here if needed await deleteDoc(dR); showToast('Demanda excluída!'); closeDemandaDetailsModal(); } } catch (error) { console.error("Erro:", error); showToast('Erro ao excluir.', 'error'); } finally { confirmDeleteBtn.disabled = false; confirmDeleteBtn.textContent = 'Excluir'; closeConfirmationModal(); }
     }

    // --- Map Logic ---
    function initializeMap() { /* ... same as your pasted version ... */ if(map) map.remove(); try { map = L.map('map').setView([-14.2350, -51.9253], 4); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OSM' }).addTo(map); markers = []; } catch(e){ console.error("Map init error:", e); showToast("Erro mapa.", "error"); document.getElementById('map').innerHTML='Erro mapa.'; } }
    async function openMapModal(cidadaosToPlot = null) { /* ... same as your pasted version ... */ mapModal?.classList.remove('hidden'); if (!map) { setTimeout(initializeMap, 100); } else { markers.forEach(m => m.remove()); markers = []; } await new Promise(r => setTimeout(r, 150)); map?.invalidateSize(); const cidadaos = cidadaosToPlot || getFilteredCidadaos(); const bounds = []; const promises = cidadaos.map(async c => { if (c.logradouro && c.cidade) { try { const addr = `${c.logradouro}, ${c.numero||''}, ${c.bairro}, ${c.cidade}, ${c.estado}`; const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(addr)}&format=json&limit=1`); const data = await res.json(); if (data && data.length > 0) { const {lat,lon} = data[0]; const latLng = [parseFloat(lat), parseFloat(lon)]; const marker = L.marker(latLng).addTo(map).bindPopup(`<strong>${c.name}</strong><br>${addr}`); markers.push(marker); bounds.push(latLng); } } catch (e) { console.warn(`Geocode error ${c.name}:`, e); } } }); await Promise.all(promises); if (bounds.length > 0) map.fitBounds(bounds, { padding: [50, 50] }); else if (!cidadaosToPlot) map.setView([-14.2350, -51.9253], 4); }
    function closeMapModal() { /* ... same as your pasted version ... */ mapModal?.classList.add('hidden'); }

    // --- Report Logic ---
    function generatePrintReport() { /* ... same as your pasted version ... */
        const filteredCidadaos = getFilteredCidadaos(); if (filteredCidadaos.length === 0) { showToast("Nenhum cidadão para relatório.", "warning"); return; } const reportWindow = window.open('', '', 'width=800,height=600'); reportWindow.document.write('<html><head><title>Relatório</title><style>body{font-family:Arial,sans-serif;margin:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#f2f2f2}h1{text-align:center;border-bottom:2px solid #000;padding-bottom:10px}@media print{button{display:none}}</style></head><body>'); reportWindow.document.write(`<h1>Relatório Cidadãos (${filteredCidadaos.length})</h1><button onclick="window.print()">Imprimir</button><table><thead><tr><th>Nome</th><th>Tipo</th><th>Telefone</th><th>Email</th><th>Endereço</th></tr></thead><tbody>`); filteredCidadaos.forEach(c=>{ const addr=[c.logradouro,c.numero,c.complemento,c.bairro,c.cidade,c.estado,c.cep].filter(Boolean).join(', ')||'-'; reportWindow.document.write(`<tr><td>${c.name}</td><td>${c.type}</td><td>${c.phone||''} ${c.whatsapp?'(W)':''}</td><td>${c.email}</td><td>${addr}</td></tr>`); }); reportWindow.document.write('</tbody></table></body></html>'); reportWindow.document.close();
     }


    // --- Utils ---
    function switchPage(pageId) { /* ... same as your pasted version ... */ document.querySelectorAll('.page').forEach(p => p.classList.add('hidden')); document.getElementById(pageId)?.classList.remove('hidden'); document.getElementById(pageId)?.classList.add('flex', 'flex-col'); document.querySelectorAll('#sidebar-nav a').forEach(l => { l.classList.remove('bg-slate-900','font-semibold'); if (l.getAttribute('href') === `#${pageId.replace('-page','')}`) l.classList.add('bg-slate-900','font-semibold'); }); if (pageId === 'dashboard-page') updateDashboard(); }
