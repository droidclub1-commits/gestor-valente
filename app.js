...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleCidadaoFormSubmit(e) {
        ...
        // A checagem de duplicidade agora é feita pelo Banco de Dados (cpf UNIQUE, voterid UNIQUE)

        try {
            let photoUrl = cidadaoPhotoUrl.value;
            const file = cidadaoPhotoUpload.files[0];

            // 1. Upload da Foto (se houver) - Lógica do Supabase Storage
            if (file) {
                const filePath = `${user.id}/${Date.now()}_${file.name}`;
                const { error: uploadError } = await sb.storage
                    .from('fotos-cidadaos') // Nome do "bucket" que criamos
            ...
            // 2. Montar objeto do Cidadão (padrão SQL: snake_case)
            const cidadaoData = {
                name: cidadaoName.value,
                email: cidadaoEmail.value,
                dob: cidadaoDob.value || null, // Nulo se vazio
                ...
                children: getChildrenData(), // JSONB aceita objetos JS
                photoUrl: photoUrl || null,
                updated_at: new Date().toISOString(), // Data de atualização
                user_id: user.id // Garantir que o dono está correto
            };
            ...
            if (currentEditingId) {
                ...
                // Criar
                // Remover 'updated_at' para deixar o DB usar o 'default'
    ...
    function closeDemandaDetailsModal() {
        ...
        // Não precisamos mais do 'notesUnsubscribe'
    }
