...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupFirestoreListeners() {
        ...
        const qCidadaos = query(cidadaosRef); // TODO: Add where("ownerId", "==", currentUserId) if needed by rules
        ...
        const qDemandas = query(demandasRef, orderBy("createdAt", "desc")); // TODO: Add where("ownerId", "==", currentUserId)
    ...
    function clearData() {
        ...
        // Destroy charts
        [cidadaosChart, demandasChart, cidadaosBairroChart, cidadaosSexoChart, cidadaosFaixaEtariaChart].forEach(chart => chart?.destroy()); // Renamed variables
        cidadaosChart = demandasChart = cidadaosBairroChart = cidadaosSexoChart = cidadaosFaixaEtariaChart = null; // Renamed variables
    }
    ...
    function updateDashboard() {
        if(!document.getElementById('dashboard-page') || document.getElementById('dashboard-page').classList.contains('hidden')) return; // Update only if dashboard is visible
    ...
    // --- Chart Functions --- (Renamed variables to avoid conflict)
    ...
    // --- Cidadao Form Logic ---
    async function handleCidadaoFormSubmit(e) { /* ... same structure, uses root 'cidadaos' ... */
    ...
    function openDetailsModal(cidadaoId) { /* ... same ... */
        ...
        // Simplified filling logic using querySelectorAll and dataset (example)
        detailsModal.querySelectorAll('[id^="details-"]').forEach(el => {
             const key = el.id.replace('details-', '').replace('-', ''); // Convert 'details-local-trabalho' to 'localTrabalho'
             ...
             else if (key === 'voterid') value = cidadao.voterId ? formatVoterID(cidadao.voterId) : '-'; // Correct key
             ...
             else if (key === 'children') { /* Handle children separately */ }
             else if (el.tagName === 'SPAN') el.textContent = value; // Update span content
        });
        // Handle photo and children specifically
    ...
    function closeDetailsModal() { /* ... same ... */
    ...
    function closeDemandaModal() { /* ... same ... */ demandaModal?.classList.add('hidden'); }
    async function handleDemandaFormSubmit(e) { /* ... same ... */
    ...
    function closeDemandaDetailsModal() { /* ... same ... */
    ...
    function closeConfirmationModal() { /* ... same ... */ document.getElementById('confirmation-modal')?.classList.add('hidden'); itemToDelete = { id: null, type: null }; }
    ...
    function closeMapModal() { /* ... same ... */ mapModal?.classList.add('hidden'); }

    // --- Report Logic ---
    function generatePrintReport() { /* ... same ... */
