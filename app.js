...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const logoBtn = document.getElementById('logo-btn'); // Logo ainda existe no HTML, mas sem listener
    ...
    const generateReportBtn = document.getElementById('generate-report-btn'); // Botão de relatório
    ...
    function initializeMainApp() {
        ...
        // [RESTAURADO] Report button calls generateCSV
    ...
    function setupFirestoreListeners() {
        ...
        const qDemandas = query(demandasRef); // Query sem orderBy
        ...
        demandasListener = onSnapshot(qDemandas, (snapshot) => {
             ...
             console.log("Firestore: Demandas updated (client sort) - ", allDemandas.length);
             const currentPageId = document.querySelector('.page:not(.hidden)')?.id?.replace('-page', '');
    ...
    // --- Cidadao Form Logic ---
    async function handleCidadaoFormSubmit(e) { /* ... same ... */
    ...
    async function handleCEPBlur(e) { /* ... same ... */
    ...
    function updateChildrenInputs(type, childrenData = null) { /* ... same ... */
    ...
    // --- Details Modal ---
    function openDetailsModal(cidadaoId) { /* ... same ... */
        ...
        detailsModal.querySelectorAll('[id^="details-"]').forEach(el => {
             const keyOriginal = el.id.replace('details-', ''); const key = keyOriginal.replace(/-/g, ''); let value = cidadao[key] ?? '-';
             if (keyOriginal === 'phone') value = cidadao.phone ? `${formatPhone(cidadao.phone)} ${cidadao.whatsapp ? '(W)' : ''}` : '-';
             else if (keyOriginal === 'address') value = [cidadao.logradouro, cidadao.numero, cidadao.complemento, cidadao.bairro, cidadao.cidade, cidadao.estado, cidadao.cep?`CEP: ${formatCEP(cidadao.cep)}`:null].filter(Boolean).join(', ')||'-';
             else if (keyOriginal === 'cpf') value = cidadao.cpf ? formatCPF(cidadao.cpf) : '-';
             else if (keyOriginal === 'voterid') value = cidadao.voterId ? formatVoterID(cidadao.voterId) : '-';
             else if (keyOriginal === 'dob') value = cidadao.dob ? formatarData(cidadao.dob) : '-';
             else if (keyOriginal === 'leader') { const leader = allLeaders.find(l=>l.name === cidadao.leader); value = leader?.name || cidadao.leader || '-';}
             else if (keyOriginal === 'local-trabalho') value = cidadao.localTrabalho || '-';
             else if (keyOriginal === 'children') { /* Handle children */ }
    ...
    // --- Demanda Form/Modals ---
    function openDemandaModal(cidadaoId = null) { /* ... same ... */
    ...
    async function handleDemandaFormSubmit(e) { /* ... same, no ownerId ... */
        ...
        try {
            const data = { cidadaoId: demandaCidadaoSelect.value, title: document.getElementById('demanda-title').value, description: document.getElementById('demanda-description').value, status: 'pending', createdAt: Timestamp.now(), updatedAt: Timestamp.now() }; // No ownerId
            await addDoc(collection(db, 'demandas'), data); showToast("Demanda adicionada!", "success"); closeDemandaModal();
         } catch (error) { console.error("Erro:", error); showToast("Erro ao salvar.", "error"); } finally { saveBtnDem.disabled = false; }
     }

    // --- Demanda Details Modal ---
    async function openDemandaDetailsModal(demandaId) { /* ... same ... */
    ...
    async function updateDemandaStatus(demandaId, newStatus) { /* ... same ... */
    ...
    function loadDemandaNotes(demandaId) { /* ... same ... */
    ...
    async function handleAddNoteSubmit(e) { /* ... same ... */
    ...
    async function handleDeleteConfirmation() { /* ... same ... */
    ...
    function generateCSV() {
        ...
        if (filteredCidadaos.length === 0) {
            showToast("Nenhum cidadão encontrado para gerar CSV.", "warning");
            return;
        }

        // Cabeçalho CSV
        const headers = [
            "Nome", "Tipo", "Email", "Telefone", "WhatsApp",
            "Endereço Completo", // Coluna combinada
            "Liderança", "Data Nasc.", "Sexo", "Profissão", "Local Trabalho",
            "CPF", "RG", "Título Eleitor", "Filhos", "Filhas"
        ];
        ...
        const escapeCSV = (field) => {
            ...
            // Se contiver vírgula, ponto e vírgula, aspas ou quebra de linha, envolver em aspas duplas
            if (str.search(/("|,|;|\n)/g) >= 0) {
                str = `"${str.replace(/"/g, '""')}"`; // Escapar aspas duplas internas
            }
            return str;
        };
        ...
        filteredCidadaos.forEach(c => {
            ...
            const row = [
                c.name, c.type, c.email, c.phone ? formatPhone(c.phone) : '', c.whatsapp ? 'Sim' : 'Não',
                enderecoCompleto,
                c.leader || '', // Usar nome da liderança
                c.dob ? formatarData(c.dob) : '',
                c.sexo || 'Não Informar',
                c.profissao || '', c.localTrabalho || '',
                c.cpf ? formatCPF(c.cpf) : '', c.rg || '', c.voterId ? formatVoterID(c.voterId) : '', // Corrigido para voterId
        ...
        link.setAttribute("download", "relatorio_cidadaos.csv");
