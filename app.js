...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeMainApp() {
        ...
        function generateCSV(data) {
            // [ALTERAÇÃO] - Cabeçalho do Endereço
            const headers = [
                "Nome", "Email", "Data Nasc.", "Sexo", "Tipo", "CPF", "RG", "Título Eleitor", 
                "Telefone", "WhatsApp", "Profissão", "Local Trabalho", "Liderança", 
                "Endereço Completo"
            ];
            const rows = data.map(c => {
                ...
                // [ALTERAÇÃO] - Concatenar o endereço
                const fullAddress = [
                    addr.logradouro || '',
                    ...
                    addr.estado || '',
                    addr.cep || ''
                ].filter(Boolean).join(', '); // Remove partes vazias e junta com vírgula

                return [
                    c.name, c.email, c.dob || '', c.sexo || 'N/A', c.type, c.cpf || '', c.rg || '', c.voterId || '',
                    c.phone || '', c.hasWhatsApp ? 'Sim' : 'Não', c.profissao || '', c.localTrabalho || '',
                    leader ? leader.name : 'Nenhuma',
                    fullAddress // Usa a string do endereço completo
        ...
        generateReportBtn.addEventListener('click', () => {
            const csvContent = generateCSV(allCidadaos);
